...
class IRGalleryEditActivity : BaseActivity(), View.OnClickListener, ITsTempListener {
    ...
    private var struct: FrameStruct = FrameStruct() //[Chinese text]
...
//        adapter.enPseudoColorBar(struct.isShowPseudoBar)
            ...
            updateTemperatureSeekBar(false, R.drawable.svg_pseudo_bar_lock, "lock")//[Chinese text]
            ...
            temperature_seekbar.setRange(leftValue, rightValue, 0.1f) //[Chinese text]temperaturerange
            temperature_seekbar.setProgress(leftValue, rightValue) //[Chinese text]
    ...
    private fun initRecycler() {
        edit_recycler_first.onTabClickListener = {
            when (it) {
                0 -> edit_recycler_second.selectPosition(1) //pointline[Chinese text]
                1 -> edit_recycler_second.selectPosition(3) //[Chinese text]
