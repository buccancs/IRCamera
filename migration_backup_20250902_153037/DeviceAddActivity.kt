...
class DeviceAddActivity : BaseActivity() {
    ...
    /**
     * [Chinese text], [Chinese text] TS004 [Chinese text] TC007.
     */
    ...
    /**
     * [Chinese text], [Chinese text], [Chinese text] WIFI [Chinese text]text, [Chinese text], [Chinese text]
     */
    ...
    override fun initView() {
        ...
        LocationUtil.addBtStateListener(this) {
            // [Chinese text], [Chinese text]
            XLog.i("[[Chinese text]] [Chinese text]: ${if (it) "[Chinese text]" else "[Chinese text]"}")
            refreshStateAndTips()
            if (it) {
                if (isFirstRequest) {
                    requestPermission(2)
        ...
        WifiUtil.addWifiStateListener(this) {
            // [Chinese text] WIFI [Chinese text], [Chinese text] WIFI [Chinese text]
            XLog.i("[[Chinese text]] WIFI [Chinese text]: ${if (it) "[Chinese text]" else "[Chinese text]"}")
            refreshStateAndTips()
            if (it) {
                if (isFirstRequest) {
                    requestPermission(2)
    ...
    /**
     * [Chinese text]in progress[Chinese text]text.
     */
            ...
            openLocationDialog = TipDialog.Builder(this)
                ...
                .setPositiveListener(R.string.app_open) {
                    ...
                    if (intent.resolveActivity(packageManager) == null) {
                        XLog.e("[[Chinese text]] [Chinese text] Intent [Chinese text] Activity, [Chinese text]settings[Chinese text]")
                        intent = Intent(Settings.ACTION_SETTINGS)
                        if (intent.resolveActivity(packageManager) == null) {
                            XLog.e("[[Chinese text]] [Chinese text]! [Chinese text]settings[Chinese text] Intent [Chinese text] Activity!")
    ...
    /**
     * [Chinese text] WIFI [Chinese text], [Chinese text], [Chinese text].
     */
        ...
        if (openBtDialog == null) {
            openBtDialog = TipDialog.Builder(this)
                ...
                .setPositiveListener(R.string.app_open) {
                    ...
                    if (intent.resolveActivity(packageManager) == null) {
                        XLog.e("[[Chinese text]] [Chinese text]! [Chinese text] Intent [Chinese text] Activity! [Chinese text]settings[Chinese text]")
                        intent = Intent(Settings.ACTION_SETTINGS)
                        if (intent.resolveActivity(packageManager) == null) {
                            XLog.e("[[Chinese text]] [Chinese text]! [Chinese text]settings[Chinese text] Intent [Chinese text] Activity!")
                            return@setPositiveListener
    ...
    /**
     * [Chinese text] WIFI [Chinese text].
     */
                ...
                .setPositiveListener(R.string.app_open) {
                    if (Build.VERSION.SDK_INT < 29) {//low[Chinese text] Android10
                        ...
                        if (wifiIntent.resolveActivity(packageManager) == null) {
                            XLog.e("[[Chinese text]] [Chinese text] WIFI Intent [Chinese text] Activity! [Chinese text] WIFI settings Intent")
                            wifiIntent = Intent(Settings.ACTION_WIFI_SETTINGS)
                            if (wifiIntent.resolveActivity(packageManager) == null) {
                                XLog.e("[[Chinese text]] [Chinese text]! WIFI settings Intent [Chinese text] Activity! [Chinese text]settings[Chinese text]")
    ...
    /**
     * [Chinese text], [Chinese text].
     * @param actionType [Chinese text] 0-[Chinese text] 1-only[Chinese text] 2-[Chinese text]
     */
        ...
        PermissionTool.requestBluetooth(this, actionType == 1, object : PermissionTool.Callback {
            override fun onResult(allGranted: Boolean) {
                ...
                if (allGranted) {
                    ...
                    if (actionType == 1) {
                        if (!btAdapter.isEnabled) {//[Chinese text]
                            showOpenBtDialog()
                            return
                        }
                    } else {
                        if (Build.VERSION.SDK_INT >= 28 && !locationManager.isLocationEnabled) {//[Chinese text]
                            showOpenLocationDialog()
                            return
                        }
                        if (!wifiManager.isWifiEnabled) {//WIFI [Chinese text]
                            showOpenWifiDialog()
                            return
                        }
                        if (!btAdapter.isEnabled) {//[Chinese text]
    ...
    /**
     * start[Chinese text], [Chinese text], [Chinese text] return.
     */
    private fun startBtScan() {
        if (!PermissionTool.hasBtPermission(this)) {//[Chinese text]
            return
        }
        if (Build.VERSION.SDK_INT >= 28 && !locationManager.isLocationEnabled) {//[Chinese text]
            return
        ...
        //[Chinese text] WIFI [Chinese text], [Chinese text]
    ...
    private fun connectWIFI(wifiName: String) {
        if (WifiUtil.getCurrentWifiSSID(this) == wifiName && WebSocketProxy.getInstance().isConnected()) {//[Chinese text]
        ...
        XLog.i("[Chinese text] ${WifiUtil.getCurrentWifiSSID(this)} [Chinese text] $wifiName")
        showCameraLoading()
        // [Chinese text] onAvailable [Chinese text] onUnavailable [Chinese text], 15[Chinese text] Loading [Chinese text] dismiss, [Chinese text]
        // [Chinese text] connectWifi [Chinese text]in progress[Chinese text] listener [Chinese text], [Chinese text], [Chinese text], [Chinese text]
    ...
    /**
     * [Chinese text], [Chinese text], [Chinese text], [Chinese text] WIFI [Chinese text]text, [Chinese text], [Chinese text]
     */
    private class TopTipHolder(val textView: TextView) {
        ...
        enum class State {
            NONE,                //[Chinese text]
            LOCATION_PERMISSION, //[Chinese text]text
            LOCATION_INFO,       //[Chinese text]text
            BLUETOOTH_PERMISSION,//[Chinese text]text
            BLUETOOTH_SWITCH,    //[Chinese text]text
            WIFI_SWITCH,         //[Chinese text]WIFI[Chinese text]text
        }
    }
