...
def demonstrate_refined_features():
    ...
    print(f"✨ Enhanced Protocol Manager")
    ...
    print(f"   ⏱️  Heartbeat Interval: {config.get('network', {}).get('heartbeat_interval', 30)}s")
    print(f"   💾 Data Root: {config.get('data', {}).get('root_directory', './data')}")
    print(f"   📊 Session Retention: {config.get('data', {}).get('session_retention_days', 30)} days")
    print()
