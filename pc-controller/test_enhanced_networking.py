...
class EnhancedNetworkingTests:
    ...
    async def test_security_manager(self):
        ...
        try:
            ...
            logger.info("[CHECK] Security manager initialization")
            ...
            logger.info("[CHECK] Auth token generation and validation")
            ...
            logger.info("[CHECK] Token expiry handling")

            # Test token cleanup
            security_manager.cleanup_expired_tokens()
            logger.info("[CHECK] Token cleanup")
    ...
    async def test_discovery_service(self):
        ...
        try:
            ...
            logger.info("[CHECK] Device type determination")
            ...
            logger.info("[CHECK] Discovery listener management")
            ...
            if not discovery_service._check_zeroconf_available():
                ...
                logger.info("[CHECK] Fallback discovery")
            else:
                logger.info("[CHECK] Zeroconf available for full testing")
    ...
    async def test_reliable_messaging(self):
        ...
        try:
            ...
            logger.info("[CHECK] Transport configuration")
            ...
            logger.info("[CHECK] Service initialization")
            ...
            logger.info("[CHECK] Message handler registration")
            ...
            logger.info("[CHECK] Message sending")

            # Wait for message processing
            await asyncio.sleep(1)
            assert len(sent_messages) > 0, "Message not sent by transport"
            logger.info("[CHECK] Message processing")
            ...
            logger.info("[CHECK] Acknowledgment handling")
            ...
            logger.info("[CHECK] Queue size monitoring")
            ...
            logger.info("[CHECK] Service shutdown")
    ...
    async def test_network_server_integration(self):
        ...
        try:
            ...
            with (
                ...
                logger.info("[CHECK] Enhanced services initialization")

                # Test server configuration
                assert (
                    server._secure_port == server._port + 1
                ), "Secure port not configured correctly"
                logger.info("[CHECK] Server configuration")
                ...
                logger.info("[CHECK] Enhanced message handlers")
