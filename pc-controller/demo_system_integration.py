...
def demonstrate_bluetooth_features():
    ...
    try:
        from ircamera_pc.core.bluetooth_manager import (
            BluetoothManager, BluetoothDevice, BluetoothDeviceType, ConnectionState
        )
        ...
        print_feature("IRCamera Detection", "Automatic identification of IRCamera devices")
        ...
        print_feature("Cross-Platform", f"Windows/Linux/macOS support (current: {platform_module.system()})")
...
def demonstrate_wifi_features():
    ...
    try:
        from ircamera_pc.core.wifi_manager import (
            WiFiManager, NetworkSecurityType, HotspotState, WiFiNetwork
        )
        ...
        print_feature("Mobile Hotspot", "Create PC hotspot for direct device connection")
...
def demonstrate_admin_privileges():
    ...
    try:
        from ircamera_pc.core.admin_privileges import (
            AdminPrivilegesManager, PrivilegeLevel, ElevationResult, SystemPermissions
        )
...
def demonstrate_protocol_extensions():
    ...
    try:
        ...
        integration_messages = [
            "bluetooth_scan_request", "bluetooth_scan_result", 
            "bluetooth_connect_request", "bluetooth_connection_status",
            "wifi_scan_request", "wifi_scan_result",
            "wifi_connect_request", "wifi_connection_status", 
            "hotspot_control_request", "hotspot_status"
        ]
        ...
        try:
            ...
            print(f"   Sample message validation: {'✅ Valid' if is_valid else '❌ Invalid'}")
        except Exception as e:
...
def demonstrate_gui_integration():
    ...
    try:
        print_feature("Enhanced Main Window", "Three-pane layout with system integration")
...
async def main():
    ...
    summary_features = [
        ("Bluetooth Device Management", "Discover and connect to IR cameras via Bluetooth"),
