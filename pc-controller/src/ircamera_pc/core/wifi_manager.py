...
try:
    ...
    logger.warning("psutil not available. Install 'psutil' for network interface monitoring")
...
class WiFiScanWorker(BaseThread):
    ...
    def _parse_windows_scan(self, output: str) -> List[WiFiNetwork]:
        ...
        for line in lines:
            ...
            if line.startswith('SSID'):
                ...
                current_network = {'ssid': ssid_match.group(1) if ssid_match else 'Unknown'}
    ...
    def _is_ircamera_network(self, ssid: str) -> bool:
        ...
        ircamera_patterns = [
            "ircamera", "thermal", "flir", "seek", "hikvision",
            "thermal_cam", "ir_cam", "heatcam"
        ]
...
class WiFiManager(BaseManager):
    ...
    async def connect_to_network(self, ssid: str, password: str = None) -> bool:
        ...
        try:
            success = await self._platform_connect(ssid, password, network.security_type)
    ...
    async def start_hotspot(self, 
        ...
        try:
            ...
            if success:
                ...
                self.hotspot_state_changed.emit(self._hotspot_state, "Failed to start hotspot")
                return False
    ...
    def _is_wifi_interface(self, name: str) -> bool:
        ...
        wifi_patterns = [
            "wlan", "wifi", "wireless", "wi-fi", "wlp", "wl",
            "ath", "ra", "rtl", "iwl", "bnep"
        ]
    ...
    async def _platform_connect(self, ssid: str, password: str, security: NetworkSecurityType) -> bool:
    ...
    async def _connect_windows(self, ssid: str, password: str, security: NetworkSecurityType) -> bool:
    ...
    async def _connect_linux(self, ssid: str, password: str, security: NetworkSecurityType) -> bool:
        """Connect to WiFi on Linux using NetworkManager."""
        logger.info(f"Connecting to {ssid} on Linux")
        return True  # Placeholder
    
    async def _connect_macos(self, ssid: str, password: str, security: NetworkSecurityType) -> bool:
