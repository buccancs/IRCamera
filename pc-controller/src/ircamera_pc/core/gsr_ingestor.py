...
class GSRIngestor:
    ...
    async def start_session(self, session_id: str, device_id: str, mode: GSRMode) -> bool:
        ...
        try:
            ...
            logger.info(f"Started GSR session {session_id} for device {device_id} in {mode.value} mode")
            return True
    ...
    async def end_session(self, session_id: str) -> Optional[GSRDataSet]:
        ...
        try:
            ...
            logger.info(f"Sample rate: {dataset.sample_rate:.1f} Hz, Quality: {dataset.quality_stats['mean']:.1f}")
    ...
    def _validate_sample(self, sample: GSRSample, dataset: GSRDataSet) -> bool:
        ...
        if sample.quality < self.quality_threshold:
            logger.debug(f"GSR sample below quality threshold: {sample.quality} < {self.quality_threshold}")
            return False
