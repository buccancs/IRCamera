...
class TimeSyncService:
    ...
    - Runs SNTP-like service for Android devices to calibrate against PC's reference clock
    ...
    def handle_sync_request(self, device_id: str, request_data: bytes, addr: Tuple[str, int]) -> bytes:
        """
        ...
        try:
            ...
            # Create response
            response = struct.pack('!QQ', 
                                 int(client_send_time * 1000),  # Echo client time
                                 server_time_ms)  # Server time
    ...
    def _update_device_stats(self, device_id: str, client_time: float, server_time: float) -> None:
        ...
        if stats.median_offset_ms > self._target_accuracy_ms:
            logger.warning(f"Device {device_id} median offset {stats.median_offset_ms:.1f}ms "
                         f"exceeds target {self._target_accuracy_ms}ms")
        
        if stats.p95_offset_ms > self._max_offset_ms:
            logger.warning(f"Device {device_id} p95 offset {stats.p95_offset_ms:.1f}ms "
                         f"exceeds threshold {self._max_offset_ms}ms")
        
        logger.debug(f"Time sync stats for {device_id}: "
    ...
    def is_device_synchronized(self, device_id: str) -> bool:
        ...
        # Check accuracy
        return (stats.median_offset_ms <= self._target_accuracy_ms and 
                stats.p95_offset_ms <= self._max_offset_ms)
    ...
    def get_synchronization_quality(self) -> Dict[str, any]:
        ...
        synchronized_count = sum(1 for device_id in self._device_stats 
        ...
        return {
            ...
            'synchronization_rate': synchronized_count / len(self._device_stats) if self._device_stats else 0.0,
