    ...
    def _format_device_status(self, device_info: DeviceInfo) -> str:
        ...
        if device_info.last_heartbeat:
            try:
                ...
                time_since = (datetime.now().replace(tzinfo=last_hb.tzinfo) - last_hb).total_seconds()
...
class SessionControlWidget(QWidget):
    ...
    def _update_button_states(self) -> None:
        ...
        has_idle_session = (self._current_session and 
                           self._current_session.state == SessionState.IDLE.value)
        
        is_recording = (self._current_session and 
                       self._current_session.state in [
        ...
        self.new_session_btn.setEnabled(
            ...
            self._current_session.state in [SessionState.COMPLETED.value, SessionState.ERROR.value]
        )
...
class StatusDisplayWidget(QWidget):
    ...
    def update_time_sync_stats(self, stats: Dict[str, Any]) -> None:
        ...
        if total_devices > 0:
            self.sync_quality_label.setText(f"{sync_devices}/{total_devices} devices ({sync_rate:.0f}%)")
