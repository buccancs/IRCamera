...
class PerformanceTest {
    ...
    @Test
    fun testHighThroughputSyncMarkers() =
        runTest {
            ...
            syncMarkerCounts.forEach { count ->
                ...
                assertTrue(
                    "Throughput should be at least 50 ops/sec for $count markers",
                    metrics.throughputOpsPerSec >= 50.0,
                )
                assertTrue(
                    "Average latency should be under 20ms for $count markers",
                    metrics.averageLatencyMs < 20.0,
                )
                assertTrue(
                    "Error rate should be under 1% for $count markers",
    ...
    @Test
    fun testNetworkCommunicationPerformance() =
        runTest {
            ...
            messageSizes.forEach { size ->
                ...
                assertTrue(
                    "Network throughput should be reasonable for ${size}B messages",
                    metrics.throughputOpsPerSec >= 10.0,
                )
                assertTrue(
                    "Network latency should be under 100ms for ${size}B messages",
            ...
            messageResults.zipWithNext { current, next ->
                ...
                assertTrue(
                    "Latency increase should be proportional to message size",
    ...
    @Test
    fun testTimeSynchronizationPerformance() =
        runTest {
            ...
            syncCounts.forEach { count ->
                ...
                assertTrue(
                    "Time sync should maintain high accuracy under load",
                    metrics.errorRate < 0.05,
                ) // Less than 5% sync failures
                assertTrue(
                    "Average sync latency should be reasonable",
                    metrics.averageLatencyMs < 50.0,
                ) // Under 50ms per sync
            }
    ...
    fun testMemoryUsageUnderLoad() =
        runTest {
            ...
            assertTrue(
                "Average memory growth rate should be minimal",
                kotlin.math.abs(averageGrowthRate) < 1.0,
            ) // Less than 1MB per measurement
        }

    @Test
    fun testCPUUsageMonitoring() =
        runTest {
            ...
            scenarios.forEach { (scenarioName, opsPerSec) ->
                ...
                assertTrue(
                    "CPU usage should be reasonable for $scenarioName",
                    cpuMetrics.averageCpuPercent < 80.0,
                )
                assertTrue(
                    "Peak CPU should not exceed limits for $scenarioName",
    ...
    private suspend fun measureCPUUsage(
    ...
    fun testConcurrentPerformance() =
        runTest {
            ...
            concurrencyLevels.forEach { concurrency ->
                ...
                assertTrue(
                    "Concurrent performance should be reasonable for $concurrency workers",
                    metrics.throughputOpsPerSec >= 20.0,
                )
                assertTrue(
                    "Error rate should remain low with $concurrency workers",
            ...
            // Analyze scalability
            val scalabilityFactors =
                concurrencyResults.zipWithNext { current, next ->
                    val concurrencyRatio =
                        concurrencyLevels[concurrencyResults.indexOf(next)] /
