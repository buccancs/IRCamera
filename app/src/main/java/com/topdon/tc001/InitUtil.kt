...
object InitUtil {
    fun initLog() {
        ...
        val pattern = "{d}, {L}, {t}, {m}"
        val backupStrategy = FileSizeBackupStrategy2(5 * 1024 * 1024L, 10) // [Chinese text]
        val cleanStrategy = FileLastModifiedCleanStrategy(30 * 24 * 60 * 60) // Settings[Chinese text]
        ...
        val filePrinter =
            FilePrinter.Builder(fileDir) // [Chinese text]
                .fileNameGenerator(ChangelessFileNameGenerator(fileName)) // [Chinese text]
                .backupStrategy(backupStrategy) // [Chinese text]
                .cleanStrategy(cleanStrategy) // [Chinese text]
                .flattener(PatternFlattener(pattern)) // [Chinese text]
                .build()
        if (BuildConfig.DEBUG) {
            XLog.init(config, androidPrinter, filePrinter)
        } else {
            // release[Chinese text]logcat
...
//            // [Chinese text], uminit[Chinese text]1[Chinese text], [Chinese text]umsdk
//            UMConfigure.init(
...
//        }
    ...
    fun initReceiver() {
        ...
        // [Chinese text],[Chinese text]usb[Chinese text]
        val filter = IntentFilter()
        filter.addAction(UsbManager.ACTION_USB_DEVICE_ATTACHED)
        filter.addAction(UsbManager.ACTION_USB_DEVICE_DETACHED)
        filter.addAction(UsbManager.ACTION_USB_ACCESSORY_ATTACHED)
        filter.addAction(UsbManager.ACTION_USB_ACCESSORY_DETACHED)
        filter.addAction(DeviceBroadcastReceiver.ACTION_USB_PERMISSION) // [Chinese text]USB[Chinese text]
