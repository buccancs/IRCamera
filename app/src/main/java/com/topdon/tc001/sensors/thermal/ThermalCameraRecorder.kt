...
class ThermalCameraRecorder(
    ...
    private suspend fun initializeRealIRCamera(): Boolean =
        withContext(Dispatchers.IO) {
            try {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                ...
                if (index * 2 + 1 < temperatureBytes.size) {
                    ...
                    val tempRaw =
                        (
                            (temperatureBytes[index * 2].toInt() and 0xFF) or
                                ((temperatureBytes[index * 2 + 1].toInt() and 0xFF) shl 8)
                        ).toShort()
    ...
    private suspend fun setupOutputFiles() {
        ...
        val framesHeader =
            listOf("timestamp_ns", "frame_number") +
