...
class MoreHelpActivity:BaseActivity() {
    ...
    private fun initIntent() {
        ...
        if(connectionType == Constants.SETTING_CONNECTION){
            binding.tvTitle.text = getString(LibAppR.string.ts004_guide_text8)
            binding.titleView.setTitleText(LibAppR.string.ts004_guide_text6)
            ...
            binding.tvTitle.text = getString(LibAppR.string.ts004_disconnect_tips1)
            ...
            val spannable = SpannableStringBuilder(getString(LibAppR.string.ts004_disconnect_tips4))
            spannable.setSpan(UnderlineSpan(), 0, getString(LibAppR.string.ts004_disconnect_tips4).length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
    ...
    private fun startWifiList(){
        if(wifiManager.isWifiEnabled){
            ...
            TipDialog.Builder(this)
                ...
                .setMessage(LibAppR.string.ts004_wlan_tips)
