...
public class IRUVCTC {
    ...
    /**
     * @param cameraWidth     cameraWidth:256,cameraHeight:384,Graph+
     *                        cameraWidth:256,cameraHeight:192,Graph
...
//                        Log.w("3",result.maxTemperature+"///"+result.minTemperature);
//                    }catch (Exception  e){
//
//                    }
...
//                                System.arraycopy(frame, length / 2, temperatureSrc, 0, length / 2);
    ...
    private void startPreview() {
        ...
        if (CommonParams.DataFlowMode.IMAGE_AND_TEMP_OUTPUT == defaultDataFlowMode ||
             ...
             * Graph
             */
            // Y16Graph(TNRGraph，ISP)
            setFrameReady(false);
            if (isRestart) {
                if (ircmd.stopPreview(CommonParams.PreviewPathChannel.PREVIEW_PATH0) == 0) {
                    Log.i(TAG, "stopPreview complete Graph restart");
                    if (ircmd.startPreview(CommonParams.PreviewPathChannel.PREVIEW_PATH0,
                            CommonParams.StartPreviewSource.SOURCE_SENSOR,
                            ScreenUtils.getPreviewFPSByDataFlowMode(defaultDataFlowMode),
                            CommonParams.StartPreviewMode.VOC_DVP_MODE, defaultDataFlowMode) == 0) {
                        Log.i(TAG, "startPreview complete Graph restart");
                        try {
                            /*
                             * ，5840，Graph
                             * 
                        ...
                        if (ircmd.startY16ModePreview(CommonParams.PreviewPathChannel.PREVIEW_PATH0,
                            ...
                            Log.e(TAG, "startY16ModePreview error Graph restart");
                        }
                    } else {
                        Log.e(TAG, "startPreview error Graph restart");
                    }
                } else {
                    Log.e(TAG, "stopPreview error Graph restart");
                }
            } else {
                /*
                 * ISP
...
//                    }
                    // M2，startPreviewGraph，startY16ModePreviewGraph
                    if (ircmd.stopPreview(CommonParams.PreviewPathChannel.PREVIEW_PATH0) == 0) {
                    ...
                    /*
                     * TNR Graph
                     * YUVGraph，Y16Graph，，Graph
                     * ，Y16，Y16，
                     */
                    //  startY16ModePreview Graph，y16
                    if (ircmd.stopPreview(CommonParams.PreviewPathChannel.PREVIEW_PATH0) == 0) {
                        Log.i(TAG, "stopPreview complete TNR");
                        if (ircmd.startPreview(CommonParams.PreviewPathChannel.PREVIEW_PATH0,
                                CommonParams.StartPreviewSource.SOURCE_SENSOR,
