...
@SuppressLint("MissingPermission")
     ...
     *
     * Provides bluetoothmanager functionality.
     */
    public class BluetoothManager implements EventObserver {
    public static boolean iSReset = false;// [Chinese text]
    public static boolean isSending = false;// [Chinese text]
    public static boolean isClickStopCharging = false;// [Chinese text]point[Chinese text]stop[Chinese text]
    private static BluetoothManager instance = null;
    private Device mDevice;
    private Connection connection;
    public static boolean isReceiveBleData = false;// [Chinese text]
    ...
    private void setMTUValue() {
        if (mDevice.isConnected()) {
            // SettingsMTU
            Log.e("bcf_ble", "[Chinese text]: " + mDevice.getName() + "");
    ...
    private void setReadCallback() {
        if (mDevice.isConnected()) {
            ...
            LLog.w("bcf_ble", "[Chinese text]Notifycation: " + isEnabled);
            ...
            // [Chinese text]Settings[Chinese text], [Chinese text]observation[Chinese text]mode[Chinese text]
    ...
    public Connection connect(Device device) {
        ...
        connection = EasyBLE.getInstance().connect(device, config, this);// [Chinese text]listener[Chinese text], Settings[Chinese text]observation[Chinese text]
        connection.setBluetoothGattCallback(new BluetoothGattCallback() {
            @Override
            public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
                Log.d("ble_bcf_data", "[Chinese text]: status: " + status + "  [Chinese text]: " + StringUtils.toHex(characteristic.getValue()));
                setBleData("[Chinese text]: status: " + status + "  [Chinese text]: " + StringUtils.toHex(characteristic.getValue()));
            }
        });
        return connection;
    }
    ...
    public void release() {
        Log.d("bcf", "[Chinese text]BLE[Chinese text]");
    ...
    /**
     * [Chinese text]{@link Observe}[Chinese text], {@link RunOn}[Chinese text]line[Chinese text], Settings{@link Tag}[Chinese text]
     */
    ...
    public void onConnectionStateChanged(@NonNull Device device) {
        if (device.getConnectionState() != ConnectionState.SERVICE_DISCOVERED || device.getConnectionState() != ConnectionState.DISCONNECTED) {
            ...
            Log.e("wangchen", "[Chinese text]--" + device.getConnectionState());
        }
        Log.d("ywq", "MyObserver [Chinese text]: " + device.getConnectionState() + " [Chinese text]:  " + device.isConnected() + "-----[Chinese text]: " + device.getName() + "-------mac: " + device.getAddress());
...
//                setReadCallback();
    ...
    public void onConnectFailed(Device device, int failType) {
        Log.e("bcf_ble", "[Chinese text]" + device.getName());
        EventBus.getDefault().post(device.getConnectionState());
    }

    @Override
    public void onConnectTimeout(Device device, int type) {
        Log.e("bcf_ble", "[Chinese text]");
    }

    /**
     * [Chinese text]{@link Observe}[Chinese text], [Chinese text]{@link EasyBLEBuilder#setMethodDefaultThreadMode(ThreadMode)}[Chinese text]line[Chinese text]
     */
    @Observe
    @Override
    public void onNotificationChanged(@NonNull Request request, boolean isEnabled) {
        ...
        if (request.getType() == RequestType.SET_NOTIFICATION) {
            typeTag = "[Chinese text]";
            EventBus.getDefault().post(ConnectionState.MTU_SUCCESS);
        } else {
            typeTag = "Indication";
        }
        Log.d("bcf_ble", "onNotificationChanged : " + typeTag + ": " + (isEnabled ? "[Chinese text]" : "[Chinese text]"));
    }
    ...
    public boolean writeBuletoothData(byte[] data) {
        ...
        // [Chinese text] [Chinese text]BluetoothGattCallback[Chinese text]write[Chinese text]
        writeCharact.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
        writeCharact.setValue(data);
//        LLog.d("ble_bcf_data", "[Chinese text]: " + StringUtils.toHex(data));
    ...
    public void onCharacteristicRead(Request request, byte[] value) {
        // [Chinese text]
        String data = StringUtils.toHex(value); // [Chinese text]String[Chinese text]
    ...
    public void onCharacteristicChanged(Device device, UUID service, UUID characteristic, byte[] value) {
        Log.e("ble_bcf_data", "[Chinese text]: " + StringUtils.toHex(value));
