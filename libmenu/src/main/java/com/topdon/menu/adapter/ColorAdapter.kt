...
/**
 * Temperature measurement mode - menu3 - pseudo color/observation mode - menu4 - pseudo color adapter, only supports single selection.
 *
...
internal class ColorAdapter : RecyclerView.Adapter<ColorAdapter.ViewHolder>() {
     ...
     * code - pseudo color code, cannot be changed due to legacy (2D editing data, saved settings pseudo color)
     * size - preset pseudo color quantity, used by TC007
     */
    var onColorListener: ((index: Int, code: Int, size: Int) -> Unit)? = null

    /**
     * The source of this code is unknown, cannot be changed due to legacy (2D editing data, saved settings pseudo color all saved according to this)
