...
/**
 * Observation mode-菜单4-Target 菜单所用 Adapter.
 *
 * 测量模式(MODE)、Target(STYLE)、Target颜色(COLOR)、删除(DELETE)、帮助(HELP)
 *
 * - 测量模式(MODE)、Target(STYLE) 捆绑，要么都选中，要么都不选中，与 删除(DELETE) 互斥
 * - 删除(DELETE) 与 {测量模式(MODE)、Target(STYLE)、Target颜色(COLOR)} 互斥
 * - Target颜色(COLOR) 生效且未处于删除亮，颜色为默认绿色或处于删除不亮，丢给上层维护这个状态
