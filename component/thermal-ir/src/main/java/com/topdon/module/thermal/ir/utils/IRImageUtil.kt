...
object IRImageUtil {
    ...
    fun showContrast(
        ...
        try {
            ...
            val srcMat = Utils.loadResource(com.blankj.utilcode.util.Utils.getApp(), LibUiR.drawable.ic_main_menu_battery) //BGR
            val dstMat = Mat()
            Core.LUT(srcMat, lookUpTable, dstMat) //
            Core.add(dstMat, Scalar(brightness, brightness, brightness), dstMat) //
            val resultMat = Mat()
            Imgproc.cvtColor(dstMat, resultMat, Imgproc.COLOR_BGR2RGBA) //android
    ...
    private fun showSharpen(imageView: ImageView, @FloatRange(from = 0.0, to = 2.55) sharpen: Double) {
        ...
        Imgproc.cvtColor(preMat, dstMat, Imgproc.COLOR_BGR2RGBA) //android
