...
object IRCmdTool {
 ...
 parameters[length] = 1
\1bufferWhethersavedata，initializedata
 val alignByte = SharedManager.getManualData(snStr)
 System.arraycopy(alignByte, 0, parameters, calibrationDataSize + 1, alignByte.size)
 XLog.w("，Confirm")
 } catch (e: IOException) {
 ...
 */
 ...
 /**
\1set unit:cnt(128cnt = 1m, DefaultValue: 0.25 * 128 = 32)
 * @param value 0 ~ 25600
 *
\1sdksetTPD_PROP_DISTANCE
 */
 fun setTpdDis(
 irCmd: IRCMD?,
 value: Int,
 ...
 */
 ...
 /**
\1gainMode
\1@return 1:gain(Temp) 0:gain(high temperature)
 */
 ...
 /**
\1setgainMode
\1@param value 1:gain(Temp) 0:gain(high temperature)
 */
 fun setTpdGainSel(
 irCmd: IRCMD?,
 value: Int,
 ...
 */
 ...
 /**
\1enabledTemp
\1@param highC temperature，Unit
\1@param lowC temperature，Unit
 */
 fun setIsoColorOpen(
 ...
 val normalHighTemp = (highC + 273).toDouble() // Unitk
 val normalLowTemp = (lowC + 273).toDouble() // Unitk
 val highTemp = ceil(normalHighTemp * 16 * 4).toInt() // Temp
 val lowTemp = floor(normalLowTemp * 16 * 4).toInt() // Temp
 ...
 dualUVCCamera?.setTempL(tempLFin) // Temp - convert to Int
 dualUVCCamera?.setTempH(tempHFin) // Temp - convert to Int
 }
 ...
 */
 ...
 /**
\1amplification(thermal imaging)
\1ZoomScaleStep.ZOOM_STEP1: 2
\1ZoomScaleStep.ZOOM_STEP2: 4
\1ZoomScaleStep.ZOOM_STEP3: 8
\1ZoomScaleStep.ZOOM_STEP4: 16
 */
