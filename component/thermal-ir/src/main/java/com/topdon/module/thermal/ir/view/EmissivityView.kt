...
/**
 * [Chinese text] [Chinese text]used by, [Chinese text].
 *
...
class EmissivityView : View {
    ...
    /**
     * [Chinese text] Layout [Chinese text].
     */
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val firstWidth: Int = (widthSize * 135 / 335f).toInt()// 3 [Chinese text] 135:100:100
        val elseWidth: Int = (widthSize - firstWidth) / 2
        val contentWidth: Int = firstWidth + elseWidth * 2

        //[Chinese text] layoutList
        layoutList.clear()
        for (i in textList.indices) {
            val textWidth: Int = if (textList.size == 1) {
                contentWidth - SizeUtils.dp2px(24f)//left-right[Chinese text] 12dp padding
            } else {
                (if (i == 0) firstWidth else elseWidth) - SizeUtils.dp2px(24f)//left-right[Chinese text] 12dp padding
            }
            layoutList.add(
        ...
        if (maxHeight == 0) {//[Chinese text]settings[Chinese text], [Chinese text]high[Chinese text]
            maxHeight = textPaint.fontMetricsInt.bottom - textPaint.fontMetricsInt.top
        }
        maxHeight += SizeUtils.dp2px(12f)//[Chinese text] 6dp padding
