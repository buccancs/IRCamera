...
object PDFUtil {
    ...
    suspend fun delAllPDF(context: Context) = withContext(Dispatchers.IO) {
        if (Build.VERSION.SDK_INT < 29) {// [Chinese text] Android10
    ...
    suspend fun savePDF(context: Context, houseReport: HouseReport): Uri? = withContext(Dispatchers.IO) {
        ...
        val pageHeight = (pageWidth * 297f / 210f).toInt() // A4[Chinese text]high[Chinese text]210:297
        ...
        var hasUseHeight = headView.height // [Chinese text]high[Chinese text]
        val margin = SizeUtils.dp2px(6f) // [Chinese text]message[Chinese text], [Chinese text]px
        ...
        for (dirBean in houseReport.dirList) {
            ...
            if (hasAnyItem) {
                ...
                // [Chinese text]1[Chinese text]itemhigh[Chinese text]
                ...
                if (hasUseHeight + margin + titleText.height + tabTitleView.measuredHeight + tabItemView.measuredHeight > pageHeight) {// 1[Chinese text]item[Chinese text], [Chinese text]
                    pdfDocument.finishPage(page)
                    page = pdfDocument.startPage(pageInfo)
                    canvas = page.canvas
                    hasUseHeight = 0
                } else {// [Chinese text]1[Chinese text]item
                ...
                for (itemBean in dirBean.itemList) {
                    ...
                    if (hasUseHeight + tabItemView.height > pageHeight) {// 1[Chinese text]item[Chinese text], [Chinese text]
                        pdfDocument.finishPage(page)
                        page = pdfDocument.startPage(pageInfo)
                        canvas = page.canvas
            ...
            if (dataList.isEmpty()) {
                // [Chinese text], [Chinese text] 13dp [Chinese text] padding
                ...
                if (hasAnyItem) {// [Chinese text] itemShowView [Chinese text]
                    if (hasUseHeight + photoText.height + imgLineView.height > pageHeight) {// 1[Chinese text]item[Chinese text], [Chinese text]
                        pdfDocument.finishPage(page)
                        page = pdfDocument.startPage(pageInfo)
                        canvas = page.canvas
                        hasUseHeight = 0
                ...
                } else {// [Chinese text] item, [Chinese text]
                    if (hasUseHeight + margin + titleText.height + photoText.height + imgLineView.height > pageHeight) {// 1[Chinese text]item[Chinese text], [Chinese text]
                        pdfDocument.finishPage(page)
                        page = pdfDocument.startPage(pageInfo)
                        canvas = page.canvas
                        hasUseHeight = 0
                ...
                for (i in 0 until lineCount) {
                    ...
                    if (hasUseHeight + imgLineView.height > pageHeight) {// 1[Chinese text]item[Chinese text], [Chinese text]
                        pdfDocument.finishPage(page)
                        page = pdfDocument.startPage(pageInfo)
                        canvas = page.canvas
                        canvas.translate(0f, SizeUtils.dp2px(13f).toFloat())
                ...
                // [Chinese text] paddingBottom 3dp
        ...
        if (hasUseHeight + margin + footView.height > pageHeight) {// [Chinese text], [Chinese text]
            pdfDocument.finishPage(page)
            page = pdfDocument.startPage(pageInfo)
            canvas = page.canvas
        } else {
            ...
            if (Build.VERSION.SDK_INT < 29) {
                ...
                if (pdfUri != null) {
                    ...
                    if (outputStream != null) {
                        ...
                        // [Chinese text] resolver.insert() [Chinese text] Uri [Chinese text] id [Chinese text], [Chinese text], [Chinese text]
                        val selection = "${MediaStore.MediaColumns.RELATIVE_PATH} = ? AND ${MediaStore.MediaColumns.DISPLAY_NAME} = ?"
                        val selectionArgs: Array<String> = arrayOf(FileConfig.documentsDir, houseReport.getPdfFileName())
