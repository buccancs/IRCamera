 ...
 */
 ...
 XLog.d("${if (isTS004) "TS004" else "TC007"} - current：$currentVersion apk：$newVersion")
 ...
 XLog.e("${if (isTS004) "TS004" else "TC007"} - Failed! ${e.message}")
 ...
 */
 ...
 if (record == null || newVersionStr == null) { // ，current
 XLog.d("${if (isTS004) "TS004" else "TC007"} - ，current")
 firmwareDataLD.postValue(null)
 isRequest = false
 return
 }
 ...
 val currentVersion: Double = getVersionFromStr(firmware)
 XLog.d("${if (isTS004) "TS004" else "TC007"} - current：$currentVersion ：$newVersion")
 if (newVersion <= currentVersion) { // current
 firmwareDataLD.postValue(null)
 isRequest = false
 return
 ...
 */
 ...
 params.addBodyParameter("businessType", 20) // Type，20-
 ...
 */
 ...
 val valueType: Int, // 1- 2- 3-update 4-
