 ...
 */
 ...
 XLog.tag("WebSocket").d(" $currentSSID $ssid，Connection")
 ...
 */
 ...
 XLog.tag("WebSocket").d("$ssid Connection，：$reason")
 }
 ...
 XLog.tag("WebSocket").d("$ssid Failed，response: ${response?.message}")
 XLog.tag("WebSocket").d("$ssid Failed，: ${t.message}")
 ...
 XLog.tag("WebSocket").w("DisconnectConnection")
 handler.reset()
 ...
 */
 ...
 XLog.tag("WebSocket").i("，CurrentConnection WIFI：$wifiName")
 ...
 */
 ...
 XLog.tag("WebSocket").w(" $reconnectCount Failed")
 ...
 XLog.tag("WebSocket").w("Failed， ConnectionDisconnect ")
 reconnectCount = 0
 isReconnecting = false
 removeCallbacksAndMessages(null)
 getInstance().stopWebSocket()
 ...
 XLog.tag("WebSocket").d("Error，Start")
 reconnectCount = 0
 isReconnecting = true
