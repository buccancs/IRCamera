...
/**
 * OKHttpClient used by, for[Chinese text] Interceptor.
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (BuildConfig.DEBUG) {
            ...
            if (requestBody != null && (contentType == null || contentType == "application/json")) {
                ...
                XLog.tag("RetrofitLog").v("[Chinese text]: ${buffer.readString(StandardCharsets.UTF_8)}")
            }
        }
        ...
        if (BuildConfig.DEBUG) {
            ...
            if (responseBody != null && (isTC007 || contentType == null || contentType == "application/json")) {
                ...
                if (responseStr.length > 1024) {
                    XLog.tag("RetrofitLog").v("[Chinese text]: ${responseStr.substring(0, 1024)} ...[Chinese text]")
                } else {
                    XLog.tag("RetrofitLog").v("[Chinese text]: $responseStr")
                }
            }
        }
