...
abstract class BaseApplication : Application() {
    ...
    open fun initWebSocket() {
        connectWebSocket()
        //  - using modern network callback for Android 10+
    ...
    private fun parserSocketMessage(msgJson: String) {
        ...
        if (SharedManager.is04AutoSync) { // saved
            when (SocketCmdUtil.getCmdResponse(msgJson)) {
                WsCmdConstants.AR_COMMAND_SNAPSHOT -> { // capture
                    autoSaveNewest(false)
                }

                WsCmdConstants.AR_COMMAND_VRECORD -> { // recording
                    try {
