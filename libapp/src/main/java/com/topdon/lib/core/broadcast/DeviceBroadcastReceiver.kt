 ...
 */
 ...
 DeviceTools.isConnect(isSendConnectEvent = true, isAutoRequest = false) // ConfirmusbConnection
 } else {
 ...
 if (UsbManager.ACTION_USB_DEVICE_ATTACHED == intent.action) { // Connection
 DeviceTools.isConnect(isSendConnectEvent = true, isAutoRequest = true)
 }
 if (UsbManager.ACTION_USB_DEVICE_DETACHED == intent.action) { // Disconnect
