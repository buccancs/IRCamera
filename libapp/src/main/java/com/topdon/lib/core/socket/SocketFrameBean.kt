...
data class SocketFrameBean(
 ...
 if (isMaxShow) {
 stringBuilder.append("Temp ($maxX, $maxY) Temperature${maxValue.toCStr()} ${isMaxWarn.openText()}\n")
 }
 if (isMinShow) {
 stringBuilder.append("Temp ($minX, $minY) Temperature${minValue.toCStr()} ${isMinWarn.openText()}\n")
 }
 if (isCenterShow) {
 stringBuilder.append(" ($centerX, $centerY) Temperature${centerValue.toCStr()} ${isCenterWarn.openText()}\n")
 }

 if (isP1Show) {
 stringBuilder.append("1 ($p1X, $p1Y) Temperature${p1Value.toCStr()}\n")
 }
 if (isP2Show) {
 stringBuilder.append("2 ($p2X, $p2Y) Temperature${p2Value.toCStr()}\n")
 }
 if (isP3Show) {
 stringBuilder.append("3 ($p3X, $p3Y) Temperature${p3Value.toCStr()}\n")
 }

 if (isL1Show) {
 stringBuilder.append("1 ($l1StartX, $l1StartY)-($l1EndX, $l1EndY) ")
 stringBuilder.append("Temp${l1MinValue.toCStr()}($l1MinX, $l1MinY) Temp${l1MaxValue.toCStr()}($l1MaxX, $l1MaxY) ")
 stringBuilder.append("Temp${l1AveValue.toCStr()}\n")
 }
 if (isL2Show) {
 stringBuilder.append("2 ($l2StartX, $l2StartY)-($l2EndX, $l2EndY) ")
 stringBuilder.append("Temp${l2MinValue.toCStr()}($l2MinX, $l2MinY) Temp${l2MaxValue.toCStr()}($l2MaxX, $l2MaxY) ")
 stringBuilder.append("Temp${l2AveValue.toCStr()}\n")
 }
 if (isL3Show) {
 stringBuilder.append("3 ($l3StartX, $l3StartY)-($l3EndX, $l3EndY) ")
 stringBuilder.append("Temp${l3MinValue.toCStr()}($l3MinX, $l3MinY) Temp${l3MaxValue.toCStr()}($l3MaxX, $l3MaxY) ")
 stringBuilder.append("Temp${l3AveValue.toCStr()}\n")
 }

 if (isR1Show) {
 stringBuilder.append("1 ($r1StartX, $r1StartY)-($r1EndX, $r1EndY) ")
 stringBuilder.append("Temp${r1MinValue.toCStr()}($r1MinX, $r1MinY) Temp${r1MaxValue.toCStr()}($r1MaxX, $r1MaxY) ")
 stringBuilder.append("Temp${r1AveValue.toCStr()}\n")
 }
 if (isR2Show) {
 stringBuilder.append("2 ($r2StartX, $r2StartY)-($r2EndX, $r2EndY) ")
 stringBuilder.append("Temp${r2MinValue.toCStr()}($r2MinX, $r2MinY) Temp${r2MaxValue.toCStr()}($r2MaxX, $r2MaxY) ")
 stringBuilder.append("Temp${l2AveValue.toCStr()}\n")
 }
 if (isR3Show) {
 stringBuilder.append("3 ($r3StartX, $r3StartY)-($r3EndX, $r3EndY) ")
 stringBuilder.append("Temp${r3MinValue.toCStr()}($r3MinX, $r3MinY) Temp${r3MaxValue.toCStr()}($r3MaxX, $r3MaxY) ")
 stringBuilder.append("Temp${r3AveValue.toCStr()}\n")
 }
