...
class WsManager(private val wsUrl: String, private val okHttpClient: OkHttpClient, private val statusListener: IWebSocketListener) {
 ...
 private var status: State = State.DISCONNECTED // websocketConnectionStatus
 ...
 @Synchronized
 fun startConnect() {
 if (status == State.CONNECTING || status == State.CONNECTED) { // ConnectionConnection
 Log.w("WebSocket", "${if (status == State.CONNECTING) "Connection" else "Connection"} startConnect() ")
 return
 }
 ...
 if (currentTime - lastHeartBeatTime > 15 * 1000) { // 3， 5 Disconnect
 Log.d("WebSocket", "5，ConnectionDisconnect")
 ...
 Log.v("WebSocket", "--> ${if (isSuccess) "Success" else "Failed"}")
 }
 }
 }
 },
