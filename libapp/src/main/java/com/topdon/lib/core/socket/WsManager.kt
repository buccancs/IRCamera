...
class WsManager(private val wsUrl: String, private val okHttpClient: OkHttpClient, private val statusListener: IWebSocketListener) {
    ...
    fun startConnect() {
        if (status == State.CONNECTING || status == State.CONNECTED) { // 
            Log.w("WebSocket", "${if (status == State.CONNECTING) "Test Data" else "Test Data"} startConnect() Text")
            return
        }
                    ...
                    override fun run() {
                        ...
                        if (currentTime - lastHeartBeatTime > 15 * 1000) { // 3， 5 
                            ...
                            if (heartBeatMsg == null) {
                                ...
                                Log.v("WebSocket", "Test Data"Text" else "Test Data"}")
                            }
                        }
                    }
                },
